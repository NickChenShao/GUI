...
static void ShowMainMenu(MenuShow_s *pstShowInfo,uint8_t menuState)
{
    ...
    for (int i = 0; i < showNum; i++)
    {
        ...
        pMenuImage = (MenuImage_t *)pstShowInfo->pItemsExData[tmpselect];
    ...
    for (int i = 0; i < showNum; i++)
    {
        ...
        if (tmpselect == pstShowInfo->selectItem)
        {
            printf("\e[0;30;47m %-s%-8s%-s \e[0m", pMenuImage->pImage, pstShowInfo->pszItemsDesc[tmpselect], pMenuImage->pImage);
        }
        else
        {
            printf("\e[7;30;47m %-s%-8s%-s \e[0m", pMenuImage->pImage, pstShowInfo->pszItemsDesc[tmpselect], pMenuImage->pImage);
        }
    }
